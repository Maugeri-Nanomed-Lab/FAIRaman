...
FAIRaman automatically recognizes many commonly used metadata field
names and maps them to the appropriate HDF5 metadata path.

This allows users to import metadata from Excel, CSV or TXT files
without manually renaming every column.
...
For example, all of the following are interpreted as the same field:
...
# Automatic field mapping
  ...
  ------------------------------------- -------------------------------------------
  `project`, `project_name`, `study`,   `PROJECT.project_name`
  `study_name`                          
  ...
  `anatomical_site`, `site`, `tissue`   `SAMPLE.SAMPLE_INFO.anatomical_site`

  `anatomical_site_code`, `UBERON`,     `SAMPLE.SAMPLE_INFO.anatomical_site_code`
  `uberon_id`                           
  ...
  `donor`, `donor_id`, `patient`,       `SAMPLE.SAMPLE_DONOR.donor_id`
  `patient_id`                          

  `sex`, `donor_sex`                    `SAMPLE.SAMPLE_DONOR.donor_sex`

  `age`, `donor_age`                    `SAMPLE.SAMPLE_DONOR.donor_age`

  `diagnosis`, `diagnosis_notes`        `SAMPLE.SAMPLE_DONOR.diagnosis_notes`
  ...
  `exposure`, `exposure_time`,          `ENTRY.measurement.exposure_time`
  `integration_time`                    
  ...
  `accumulation`, `accumulations`       `ENTRY.measurement.accumulation_count`

  `instrument`, `instrument_name`       `ENTRY.instrument.name`

  `laser`, `laser_wavelength`,          `ENTRY.instrument.laser.wavelength`
  `laser_nm`, `wavelength`              
  ...
  `objective`, `lens`                   `ENTRY.instrument.optical_system.lens`
  ---------------------------------------------------------------------------------
...
If a metadata column already contains a valid FAIRaman HDF5 path, it is
...
If a metadata column matches the final element of a unique HDF5 path,
FAIRaman automatically assigns it.

------------------------------------------------------------------------

# Fields ignored automatically

The following fields are considered technical file-management
information and are **not** mapped automatically:
...
These fields will appear as **Do not map** in the mapping dialog.
...
Users can freely modify every mapping inside the FAIRaman metadata
import GUI before creating the HDF5 dataset.

------------------------------------------------------------------------

# Design philosophy

The mapping system is intentionally permissive to maximize
interoperability with existing laboratory spreadsheets while preserving
a single standardized FAIRaman HDF5 schema.

This approach minimizes manual preprocessing and facilitates the
