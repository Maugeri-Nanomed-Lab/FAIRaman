...
def process_txt_spectrum(txt_path: Path) -> dict:
    """
    Import ASCII/CSV → FAIRaman canonical dictionary.
    ...
    2) Multi-spectrum matrix without coordinates:
    ...
    3) Long spatial format:
       x | y | raman_shift | intensity

       Each row represents a single spectral value at a position (x, y).
       FAIRaman reconstructs a spectrum for each x/y pair.

    4) Wide spatial format with header:
       x | y | 800 | 801 | 802 | ...

       The columns from the third onwards are Raman intensities; the numerical headers
       are the Raman shifts.

    FAIRaman convention
    --------------------
    All x/y coordinates are expressed in micrometres.

    If the file does not contain physical coordinates, FAIRaman generates
    synthetic coordinates in micrometres solely to maintain an HDF5-compatible structure.
    In such cases:
       coordinate_source = ‘synthetic’
       coordinate_validated = False
    """
    ...
    def _read_ascii_table(path: Path) -> pd.DataFrame:
        """
        Reads an ASCII/CSV table by trying common delimiters.
        Use `header=None` to also recognise numerical headers for Raman shifts.
        """
        for sep in [r"\s+", ",", ";", "\t"]:
            try:
                df = pd.read_csv(
    ...
    def _sort_by_raman_shift(
        ...
        """
        Sort the Raman axis in ascending order.
    ...
    def _as_regular_grid_from_axes(
        ...
        """
        Builds a canonical regular_grid output.
        """
        out = _base_output()
    ...
    def _as_points(
        ...
        """
        Constructs a canonical output named `point_coordinates`.
        Used only when the actual x and y values do not form a complete grid.
        """
        out = _base_output()
    ...
    def _grid_or_points_from_xy(
        ...
        """
        Determines whether the x/y coordinates represent a complete grid or scattered points.

        If all combinations of x_unique × y_unique occur only once,
        the data is saved as regular_grid.

        Otherwise, it is saved as point_coordinates, with x/y values stored per spectrum.
        """
        raman_shift = np.asarray(raman_shift, dtype=np.float64)
        spectra = np.asarray(spectra, dtype=np.float64)
    ...
    # ── CASE 4: wide spatial format with header ────────────────────────────────
    # Example:
    #   x,y,800,801,802
    #   0,0,10,11,12
    #   1,0,13,14,15
    #
    # With header=None + to_numeric:
    #   first row = NaN, NaN, 800, 801, 802
